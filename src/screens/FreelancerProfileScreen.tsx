...
interface FreelancerProfileScreenProps {
  navigation: any;
  onOpenMenu: () => void;
}

export const FreelancerProfileScreen: React.FC<FreelancerProfileScreenProps> = ({ navigation: _navigation, onOpenMenu }) => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        const res = await freelancerService.getProfile();
  ...
  const handleSaveChanges = async () => {
    setSaving(true);
    try {
      const res = await freelancerService.updateProfile({ fullName, mobileNumber });
      if (res.success) {
  ...
  const handleStep3UpdatePassword = async () => {
    ...
    setPassOtpError(null);
    try {
      const passRes = await freelancerService.changePassword({ newPassword: modalNewPassword });
      if (passRes.success) {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
      ...
      <View style={[styles.headerBar, { borderBottomColor: colors.border, backgroundColor: isDark ? '#0D0E12' : '#FFFFFF' }]}>
        ...
        <View style={styles.headerRightActions}>
          <TouchableOpacity onPress={() => setShowPassModal(true)} style={styles.headerIconBtn}>
                ...
                <View style={styles.bannerInfo}>
                  ...
                  {mobileNumber ? (
                    <Text style={styles.bannerOwner}>
                      <Phone size={11} color="#FFC700" /> {mobileNumber}
                    </Text>
                  ) : null}
                </View>
              </View>
              ...
              <View style={[styles.panelHeader, { borderBottomColor: colors.border }]}>
                <View style={styles.panelHeaderTop}>
                  <View style={styles.panelHeaderIcon}>
                    <ShieldCheck size={16} color="#FFC700" />
                  </View>
                  <View style={styles.panelHeaderTitleWrap}>
                    <Text style={[styles.panelHeaderTitle, { color: colors.foreground }]}>Personal Information</Text>
