...
interface InspectorProfileScreenProps {
  navigation: any;
  onOpenMenu: () => void;
}

export const InspectorProfileScreen: React.FC<InspectorProfileScreenProps> = ({ navigation: _navigation, onOpenMenu }) => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        const res = await inspectorService.getProfile();
  ...
  const handleSaveChanges = async () => {
    setSaving(true);
    try {
      const res = await inspectorService.updateProfile({ fullName, mobileNumber });
      if (res.success) {
  ...
  const handleStep3UpdatePassword = async () => {
    ...
    setPassOtpError(null);
    try {
      const passRes = await inspectorService.changePassword({ newPassword: modalNewPassword });
      if (passRes.success) {
