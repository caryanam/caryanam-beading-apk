...
interface AdminFreelancersScreenProps {
  navigation: any;
  onOpenMenu: () => void;
}

export const AdminFreelancersScreen: React.FC<AdminFreelancersScreenProps> = ({ navigation, onOpenMenu }) => {
  ...
  const fetchFreelancers = async (showMsg = false) => {
    if (showMsg) setRefreshing(true);
    try {
      const res = await adminService.getRegisteredFreelancers();
      if (res.success && res.data) {
        ...
        setFreelancers(list);
        if (showMsg) showToast({ message: 'Freelancers list refreshed', type: 'success' });
      }
    } catch {
  ...
  const onRefresh = () => fetchFreelancers(true);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
      ...
      <View style={[styles.headerBar, { borderBottomColor: colors.border, backgroundColor: isDark ? '#0D0E12' : '#FFFFFF' }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={[styles.headerTitle, { color: colors.foreground }]}>Freelancers</Text>
          {Freelancers.length > 0 && (
            <View style={styles.countBadge}>
              <Text style={styles.countBadgeText}>{Freelancers.length}</Text>
            </View>
          )}
        </View>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { color: colors.mutedForeground }]}>Loading Freelancers...</Text>
        </View>
      ) : (
        <ScrollView
          style={{ flex: 1 }}
        ...
        >
          {filteredFreelancers.length === 0 ? (
            <View style={styles.emptyContainer}>
              ...
              <Text style={[styles.emptyTitle, { color: colors.foreground }]}>No Freelancers Found</Text>
              <Text style={[styles.emptySub, { color: colors.mutedForeground }]}>
                {searchQuery.trim()
                  ? `No Freelancer matches "${searchQuery}"`
                  : 'No registered Freelancers available yet.'}
              </Text>
            </View>
          ) : (
            filteredFreelancers.map((ins) => (
              <View
                key={ins.id}
                style={[
                  styles.card,
              ...
              >
                ...
                <View style={[styles.cardHeader, { backgroundColor: cardHeaderBg }]}>
                  <View style={styles.cardHeaderLeft}>
                    ...
                    <Text style={[styles.FreelancerName, { color: colors.foreground }]} numberOfLines={1}>
                      {ins.name}
                    </Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  FreelancerName: { fontSize: 14.5, fontWeight: '900', letterSpacing: -0.3 },
