...
interface DealerFreelancerVehicleDetailScreenProps {
  navigation: any;
  route: any;
  onOpenMenu: () => void;
}

export const DealerFreelancerVehicleDetailScreen: React.FC<DealerFreelancerVehicleDetailScreenProps> = ({ navigation, route, onOpenMenu: _onOpenMenu }) => {
  ...
  const loadDetails = async () => {
    ...
    try {
      let res: any = null;
      try {
        res = await freelancerService.getInspectionDetails(Number(targetId));
      } catch (e) {
        console.log('Freelancer details API failed, trying fallback...', e);
      }
      if (!res || (!res.success && !res.inspectionId && !res.vehicleDetails)) {
        try {
          res = await dealerService.getMarketplaceInspectionDetails(Number(targetId));
        } catch (e2) {
          console.log('Dealer marketplace API failed, trying public fallback...', e2);
        }
      }
      if (!res || !res.success || !res.data) {
        try {
          ...
          console.log('Admin inspection API failed, trying public API fallback...', e3);
      ...
      if (raw && (raw.inspectionId || raw.vehicleDetails || raw.status)) {
        ...
        setRawDetails(rawData);
        const v = rawData.vehicleDetails || {};
        const inspectionId = rawData.inspectionId || Number(vehicleId);
        ...
        const endsAtTime = v.auctionEndTime || Date.now() + 1000 * 60 * 15;
        ...
        const rawOwner = v.ownership || v.owner || v.ownerType || v.numberOfOwners || v.ownerName || v.customerName || rawDetails?.ownerName || rawDetails?.customerName || 1;
  ...
  const documentSpecs = useMemo(() => {
    if (!vehicle) return [];
    return [
      { label: 'Registration Number', value: vehicle.registrationNumber || rawDetails?.vehicleDetails?.registrationNumber || rawDetails?.vehicleDetails?.regNo || rawDetails?.vehicleNumber || 'N/A', icon: ShieldCheck },
      { label: 'Owner Profile', value: vehicle.ownership || '1st Owner', icon: User },
      { label: 'Location', value: vehicle.location || 'N/A', icon: MapPin },
      { label: 'RTO Information', value: vehicle.rtoInformation || 'N/A', icon: ShieldCheck },
      { label: 'Insurance Status & Validity', value: vehicle.insuranceStatus || 'Valid', icon: ShieldCheck },
      { label: 'Under Hypothecation', value: vehicle.underHypothecation || 'No', icon: ShieldCheck },
      { label: 'Accidental History', value: vehicle.accidental || rawDetails?.vehicleDetails?.accidental || rawDetails?.accidental || 'No', icon: TriangleAlert },
    ];
  }, [vehicle, rawDetails]);
  ...
  const detailSteps = [
    ...
    { id: 'videos', title: `Step 2: Videos & Sound (${videoList.length})`, subtitle: 'Engine noise & video clips' },
  ];
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { color: colors.mutedForeground }]}>
            Fetching Vehicle Details & Live Auction State...
          </Text>
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
      ...
      >
        <View style={styles.contentBody}>
            ...
            <TouchableOpacity onPress={() => setPreviewIndex(0)} activeOpacity={0.95} style={styles.galleryHeroWrap}>
              ...
              <View style={styles.heroBottom}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.heroSub}>Quality Verified</Text>
                </View>
          ...
          {/* SECTION 4: Detailed Inspection Report Tabs */}
          <Text style={[styles.sectionLabel, { marginTop: 16, marginBottom: 10 }]}>Detailed Vehicle Report</Text>
          ...
          {activeTab === 'car_documents' && (
            <View style={styles.stepContent}>
              <View style={[styles.panel, { backgroundColor: cardBg, borderColor: colors.border }]}>
                <View style={styles.panelHeaderRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.panelTitle, { color: colors.foreground }]}>Car Documents & Legal Status</Text>
                    <Text style={[styles.panelSub, { color: colors.mutedForeground }]}>
              ...
              <View style={[styles.panel, { backgroundColor: cardBg, borderColor: colors.border }]}>
                <View style={styles.panelHeaderRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.panelSub, { color: colors.mutedForeground }]}>
                      Inspection sound recordings and video clips.
                    </Text>
                  </View>
                </View>
