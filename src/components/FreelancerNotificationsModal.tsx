...
interface FreelancerNotificationItem {
  isRead?: boolean;
  id: number;
  rawId: number;
  title: string;
...
interface FreelancerNotificationsModalProps {
  iconColor?: string;
}

export const FreelancerNotificationsModal: React.FC<FreelancerNotificationsModalProps> = ({ iconColor }) => {
  const { theme, colors } = useTheme();
  const isDark = theme === 'dark';

  const [visible, setVisible] = useState(false);
  const [items, setItems] = useState<FreelancerNotificationItem[]>([]);
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const res = await freelancerService.getFreelancerNotifications();
      ...
      console.error('Failed to load Freelancer notifications', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const markSingleAsRead = async (rawId: number) => {
    try {
      await freelancerService.markNotificationAsRead(rawId);
      setItems((prev) =>
        prev.map((item) => (item.rawId === rawId ? { ...item, isRead: true } : item))
      );
      setUnreadCount((prev) => Math.max(0, prev - 1));
    } catch (err) {
      console.error('Failed to mark Freelancer notification as read', err);
    }
  };

  const markAllAsRead = async () => {
    try {
      await freelancerService.markAllNotificationsAsRead();
      setItems((prev) => prev.map((item) => ({ ...item, isRead: true })));
      setUnreadCount(0);
    } catch (err) {
      console.error('Failed to mark all Freelancer notifications as read', err);
    }
  };
