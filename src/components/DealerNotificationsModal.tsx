...
interface DealerNotificationItem {
  isRead?: boolean;
  id: number;
  rawId: number;
  title: string;
...
// Matches web formatIndianDateTime (see AdminNotificationsModal port)
...
const dotColorFor = (status: string): string => {
  ...
  if (s === 'REJECTED' || s.includes('REJECT')) return '#F43F5E';
  if (
    s === 'SUBMITTED' ||
    s === 'READY_FOR_AUCTION' ||
    s.includes('PENDING') ||
  ...
  if (s.includes('LIVE') || s.includes('BID')) return '#10B981';
  if (s === 'WON' || s.includes('WIN')) return '#3B82F6';
  return '#10B981';
};

interface DealerNotificationsModalProps {
  iconColor?: string;
  navigation?: any;
}

export const DealerNotificationsModal: React.FC<DealerNotificationsModalProps> = ({ iconColor, navigation }) => {
  const { theme, colors } = useTheme();
  const isDark = theme === 'dark';

  const [visible, setVisible] = useState(false);
  const [items, setItems] = useState<DealerNotificationItem[]>([]);
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (notifRes.success && notifRes.data && notifRes.data.length > 0) {
        ...
        // Fallback: derive notifications from live marketplace inspections (matches web app-shell)
        const res = await dealerService.getMarketplace();
        if (res.success && res.data) {
          list = res.data.map((ins: any) => {
      ...
      console.error('Failed to load dealer notifications', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const markSingleAsRead = async (rawId: number, navigateTo: string | null) => {
    try {
      await dealerService.markNotificationAsRead(rawId);
      setItems((prev) =>
        prev.map((item) => (item.rawId === rawId ? { ...item, isRead: true } : item))
      );
      setUnreadCount((prev) => Math.max(0, prev - 1));
    } catch (err) {
      console.error('Failed to mark dealer notification as read', err);
    }
    setVisible(false);
    if (navigateTo && navigation) {
      setTimeout(() => {
        navigation.navigate('DealerVehicleDetail', { vehicleId: Number(navigateTo) });
      }, 250);
    }
  };
  ...
  const markAllAsRead = async () => {
    try {
      await dealerService.markAllNotificationsAsRead();
      setItems((prev) => prev.map((item) => ({ ...item, isRead: true })));
      setUnreadCount(0);
    } catch (err) {
      console.error('Failed to mark all dealer notifications as read', err);
    }
  };
              ...
              {loading ? (
                ...
                items.map((n) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={n.id}
                      onPress={() => markSingleAsRead(n.rawId, n.link)}
                      activeOpacity={0.7}
                      style={[
                        styles.notifItem,
