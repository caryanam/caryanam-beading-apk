...
// Matches web formatIndianDateTime (see AdminVehiclesScreen port)
...
const dotColorFor = (status: string): string => {
  ...
  if (status === 'SUBMITTED' || status === 'READY_FOR_AUCTION') return '#F59E0B';
  return '#10B981';
};
...
export const AdminNotificationsModal: React.FC<AdminNotificationsModalProps> = ({ iconColor }) => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      let list: AdminNotificationItem[] = [];
  ...
  const markSingleAsRead = async (rawId: number) => {
    try {
      await adminService.markNotificationAsRead(rawId);
      setItems((prev) =>
